...
for i, sample in ped.iterrows():
  ...
  if paternal_id == '0' and maternal_id == '0':
    ...
    parents.append(paternal_id + maternal_id)
    dads.update({sample_id : paternal_id})
    moms.update({sample_id : maternal_id})
#    print(sample_id + " " +  paternal_id + " " + maternal_id)
    if affected_status == 1:
      if sex == 2:
...
cgen_nodes = []
for i in parents:
  pgen_nodes.append("parentnode" + str(i))
  cgen_nodes.append("childnode" + str(i))
...
# connect children to child nodes
